...
\item archivosCSV.py el cual contiene las funciones para procesar la base de datos, escribir las salidas parciales en archivos JSON y leer esos archivos
...
\item historial.py la cual nos ayuda a mantener un registro de los vuelos consultados
...
%Insertar foto
Si nosotros ponemos un objeto nulo, nos devuelve:
\begin{minted}{python}
  aeropuertoPrueba = None
  assert(aeropuertoPrueba.funcionHash(11) == 3)
\end{minted}
...
\item Se establece la llave api erronea y se solicita la informacion del aeropuerto de mexico
\end{itemize}
